...
export function useRandomState(opts?: { autoRefreshInterval?: number }) {
  ...
  const [data, { reload }] = useAsyncValue(
    async () => getRandomState(contract),
    [contract]
  );
...
export async function getRandomState(contract?: Contract) {
  ...
  try {
    ...
    const { isRolling, lastRollTime, probability, rewardAmount } = await (
      await fetch("/api/randomstate")
    ).json();
    ...
    console.error(
      "Failed to fetch random state from server, probably misconfigured."
    );
...
export function parseProbability(probability: string): BigNumber {
  return new BigNumber(1).minus(
    new BigNumber(probability).div(new BigNumber(2).pow(64))
  );
}
...
export function useProbabilities() {
  ...
  const collectiveProbability = new BigNumber(1).minus(
    new BigNumber(1).minus(probability).pow(numberOfTokens)
  );
