...
export function useWeb3ReactManager(props?: {
  ...
  const activate = useCallback(
    async (connector: AbstractConnector): Promise<void> => {
      ...
      try {
        dispatch({
          type: ActionType.CONNECTOR_ACTIVATION_STARTED,
          payload: { connector },
        });
        ...
        dispatch({
          type: ActionType.CONNECTOR_ACTIVATION_SUCCESS,
          payload: { connector, ...augmentedUpdate },
        });
      } catch (error) {
        ...
        dispatch({
          type: ActionType.CONNECTOR_ACTIVATION_FAILED,
