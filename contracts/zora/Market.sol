...
import {ZoraMedia} from "./ZoraMedia.sol";
import {IMarket} from "./IMarket.sol";
import {MahinNFT} from "../MahinNFT.sol";
...
contract Market is IMarket {
    ...
    function _finalizeNFTTransfer(uint256 tokenId, address bidder) private {
        ...
        token.safeTransfer(
            MahinNFT(mediaContract).beneficiary(),
            splitShare(bidShares.creator, bid.amount)
        );
        // Transfer bid share to previous owner of media (if applicable)
        token.safeTransfer(
            ZoraMedia(mediaContract).previousTokenOwners(tokenId),
            splitShare(bidShares.prevOwner, bid.amount)
        );

        // Transfer media to bid recipient
        ZoraMedia(mediaContract).auctionTransfer(tokenId, bid.recipient);
